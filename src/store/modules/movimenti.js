...
const mutations = {
  ...
  addDate(state, dateAggiuntive) {
    ...
    if (String(firstDataAggiunta) == chiaviDate[chiaviDate.length - 1])
      ...
      state.date[firstDataAggiunta].movimenti.concat(dateAggiuntive[
        firstDataAggiunta].movimenti)
    ...
    if (movimenti) {
      for (let index = 0; index < movimenti.length; index++) {
        ...
        if (movimento.numeroMovimento === numeroMovimento) {
          state.date[dataMovimento].movimenti[index].keyRdARapportino =
            numeroRapportino
          break
        }
      }
